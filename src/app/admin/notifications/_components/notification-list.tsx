...
import {
  formatDate,
  formatTimeAgo,
  getAdminNotificationTypeBgColor,
  getAdminNotificationTypeColor,
  getAdminNotificationTypeIcon,
} from "@/lib/utils";
import { markAllAsRead, markAsRead } from "@/actions/admin-notification";
  ...
  return (
    <div>
      <div className="mb-6 sm:mb-8">
        <div className="inline-block px-3 py-2 sm:px-4 bg-red-100 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] mb-3 sm:mb-4">
          <span className="text-xs sm:text-sm font-bold text-red-800">
            🔔 Sistem Notifikasi
          </span>
        </div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-black text-gray-900 mb-3 sm:mb-4">
          Notifikasi <span className="text-red-600">Admin</span>
        </h1>
          ...
          notifications.map((notification) => (
            <div
              ...
              className={`p-4 sm:p-6 border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] ${
                notification.isRead
                  ? "bg-white"
                  : getAdminNotificationTypeBgColor(notification.type)
              } ${!notification.isRead ? "ring-4 ring-yellow-300" : ""}`}
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div className="flex items-start flex-1">
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 ${getAdminNotificationTypeColor(
                      notification.type
                  ...
                  >
                    {getAdminNotificationTypeIcon(notification.type)}
                  </div>
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-bold border-2 border-black shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] ${
                            notification.type === "NEW_PAYMENT"
                              ...
                              : notification.type === "INSTALLATION_REQUEST"
                              ? "bg-blue-100 text-blue-800"
                              : notification.type === "USER_ACTIVATION_REQUEST"
                ...
                <div className="flex items-center justify-end sm:justify-start space-x-2 sm:ml-4">
                  <form
                    action={() => {
                      ...
                      router.push(`/admin${notification.redirectTo}`);
                    }}
                  >
                    <MarkAsReadButton />
                  </form>
