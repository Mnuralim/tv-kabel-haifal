...
export const FilterControl1 = ({ currentSortOrder }: Props) => {
  ...
  const periode = generateMonthYearList();

  const periodeOtions = [{ value: "ALL", label: "Semua Periode" }, ...periode];
  ...
  const handleSearch = useDebouncedCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.value.trim() === "") {
        ...
        replace(`/admin/reports?${newParams.toString()}`, {
          scroll: false,
        });
      } else {
        const newParams = new URLSearchParams(searchParams);
  ...
  const handleFilterPeriode = (
    ...
    if (e.target.value === "ALL") {
      newParams.delete("startPeriod");
      newParams.delete("endPeriod");
    } else {
      if (type === "start") {
        newParams.set("startPeriod", e.target.value);
      } else {
        newParams.set("endPeriod", e.target.value);
      }
    }
    replace(`/admin/reports?${newParams.toString()}`, {
      scroll: false,
        ...
        <div className="bg-neutral-50 border-4 border-neutral-700 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] rounded-none p-4 flex-1">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <select
                onChange={(e) => handleFilterPeriode(e, "start")}
            ...
            <div className="relative flex-1">
              ...
              <select
                onChange={(e) => handleFilterPeriode(e, "end")}
                className="w-full appearance-none border-2 border-neutral-700 p-2 pr-10 rounded-none focus:outline-none focus:ring-2 focus:ring-red-500"
              >
                {periodeOtions.map((option) => (
                  <option key={option.value} value={option.value}>
